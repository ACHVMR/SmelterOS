...
// =============================================================================
// LLM & VOICE ROUTES (OpenRouter + Groq Whisper + ElevenLabs)
// =============================================================================

import { getOpenRouterClient, OPENROUTER_MODELS } from '../llm/openrouter-client';
import { getVoicePipeline, getGroqWhisperClient, getElevenLabsClient, PREMIUM_VOICES, AGENT_VOICE_MAP } from '../voice';

const llmVoiceRoutes: Route[] = [
  // LLM Chat Completion
  {
    method: 'POST',
    path: '/llm/chat',
    ...
    handler: async (req: APIRequest) => {
      const { messages, model, temperature, maxTokens } = req.body as {
      ...
      if (!messages || !Array.isArray(messages)) {
        return Responses.badRequest('messages array required');
      }

      const client = getOpenRouterClient();
      const result = await client.chat(
        messages as any,
        { model: model || 'glm-4.7', temperature, maxTokens }
      );
  ...
  // LLM Simple Completion
  {
    ...
    handler: async (req: APIRequest) => {
      ...
      if (!prompt) {
        return Responses.badRequest('prompt required');
      }

      const client = getOpenRouterClient();
      const result = await client.complete(prompt, systemPrompt, { model });
  ...
  // Voice: Speech-to-Text (Groq Whisper)
  {
    ...
    rateLimit: { windowMs: 60000, maxRequests: 30 },
    handler: async (req: APIRequest) => {
      const { audio, language, model } = req.body as {
      ...
      if (!audio) {
        return Responses.badRequest('audio (base64) required');
      }

      const client = getGroqWhisperClient(model as any);
      const result = await client.transcribeBase64(audio, { language });
  ...
  // Voice: Text-to-Speech (ElevenLabs)
  {
    ...
    handler: async (req: APIRequest) => {
      ...
      if (!text) {
        return Responses.badRequest('text required');
      }

      const client = getElevenLabsClient();
      const result = agentId
        ? await client.synthesizeForAgent(agentId, text)
        : await client.synthesize({ text, voiceId });
  ...
  // Voice: Full Conversation (Audio → LLM → Audio)
  {
    method: 'POST',
    ...
    rateLimit: { windowMs: 60000, maxRequests: 20 },
    handler: async (req: APIRequest) => {
      const { audio, agentId, context, respondWithVoice } = req.body as {
        audio: string; // Base64 encoded audio
        agentId?: string;
        context?: Record<string, unknown>;
      ...
      const result = await pipeline.conversation({
        ...
        agentId,
        context,
        respondWithVoice: respondWithVoice ?? true,
      });
      ...
      return Responses.ok({
        ...
        audioResponse: result.audioResponse ? {
          ...
          audio: result.audioResponse.audio?.toString('base64'),
        } : undefined,
  ...
  // Voice: Available Voices
  {
    ...
    handler: async () => {
      ...
      return Responses.ok({
        ...
        agentVoiceMap: AGENT_VOICE_MAP,
        recommended: client.getRecommendedVoices(),
      });
    },
  },
  ...
  // Voice: Pipeline Status
  {
    ...
    handler: async () => {
      const pipeline = getVoicePipeline();
      return Responses.ok(pipeline.getStatus());
    },
  },
];
